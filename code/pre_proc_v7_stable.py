...
start_row = 100     # drop first 100 rows of error data
neg_days = 720
pos_days = 7
    ...
    for one_dir in data_dirs:
        for one_file in os.listdir(os.path.join(data_root_dir, one_dir)):
            if one_file.endswith('.csv') and not one_file.startswith('sim'):
                all_files.append(os.path.join(data_root_dir, one_dir, one_file))
    return all_files
...
def save_npz(root_name, heater_name, var):
    file_name = root_name + '/' + heater_name
    np.savez_compressed(file_name, *var)

def fill_file_wrapper(idx, file_name, total_len):
    print ("(%d/%d) start processing %s" % (idx, total_len, file_name))
    tic = time.clock()
    try:
        num_pos = fill_file(file_name, save_type='z')
    except Exception:
        num_pos = -1
    toc = time.clock()
    if num_pos == -1:
        print ("(%d/%d)[%ds] [ERROR] %s" % (idx, total_len, toc - tic, file_name))
        lock.acquire()
        with open(out_root_dir+"/error.log", "a") as efile:
            efile.write(file_name+"\n")
        lock.release()
    else:
        print ("(%d/%d)[%ds] finished %s with %d pos_days." % (idx, total_len, toc - tic, file_name, num_pos))


def fill_file(file_name, save_type='z'):
    # save type check
    ...
    if not dfa2.empty:
        for idx in dfa2.index:
            ...
            for day in xrange(all_days):
                prev_idx = idx - (day + 2)
                curr_idx = idx - (day + 1)
                if prev_idx >= 0 and dfa1.at[prev_idx, 'Operating_status:_Error_Locking'] == 0:     # 1. guarantee to exist and not error date
                    curr_date = dfa1.at[curr_idx, 'Date']
                    if curr_date >= idx_date - dt.timedelta(days=pos_days) and dfa1.at[curr_idx, 'Operating_status:_Error_Locking'] == 0:                         # 2. guarantee to be within range
                        # join to select date
                        ...
                        if save_type == 'y':
                            ...
                            dfa4.drop(['Time', 'Date'], axis=1, inplace=True)
                            x = dfa4.as_matrix()
                    ...
                    elif curr_date >= idx_date - dt.timedelta(days=all_days) and dfa1.at[curr_idx, 'Operating_status:_Error_Locking'] == 0:
                        # join to select date
                        dfa3 = df1[df1['Date'] == curr_date].copy()
        ...
        if save_type == 'z':
            ...
            save_npz(out_root_dir, heater_name, npz_array)
...
if __name__ == '__main__':
    ...
    results = [thread_pool.apply_async(fill_file_wrapper, t) for t in args]
    for r in results:
        r.get()
